                ...
                a->{
                    ...
                    try(Connection con = DriverManager.getConnection(url,"root","Ganesh@216"))
                    {
                        ...
                        try(PreparedStatement pst = con.prepareStatement(sql))
                        {
                            ...
                            pst.setString(2,s1);
                            //scince we dont need to update table only need to execute qury to filter out the user
                            ResultSet rs = pst.executeQuery();
                            if(rs.next()) //.next means theres an element scine present if will be true and execute only when true
                            {
