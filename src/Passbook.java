    ...
    Passbook(String username) {
        ...
        String sql = "select * from transactions where username = ? order by date desc";
            try(PreparedStatement pst = con.prepareStatement(sql)){
                ...
                while(rs.next()){
                    //1 row ke 4 columns ko read kiya
                    String date = rs.getString("date");
                    String desc = rs.getString("description");
                    double amt = rs.getDouble("amount");
                    double bal = rs.getDouble("balance");
                    //table model ne table name se table banaya hai table ko call karke hi save karenge
                    // data alag hai isliye pack karke rakhna padega i.e object ka array banayega
                    tableModel.addRow(new Object[]{date,desc,amt,bal});
                }
            }
        }
        catch (Exception e){
